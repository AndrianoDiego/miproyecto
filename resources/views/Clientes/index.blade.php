		...
		<div class="container">
          ...
          <div class="row">
               ...
               <div class="pull-right">
                    <a class="btn btn-success" href="{{ route('descargarPDFeventos') }}"> GENERAR PDF </a>
...
@if ($message = Session::get('Aceptado'))
     ...
     <thead class="thead-light">
          ...
          <tr>
                ...
                <td>{{ $clientes->id}} </td>
               <td>{{ $clientes->nombre1}} </td>
               <td>{{ $clientes->paterno }} </td>
               <td>{{ $clientes->materno }} </td>
               <td>{{ $clientes->edad }} </td>
               <td>{{ $clientes->dia }} </td>
               <td>{{ $clientes->evento }} </td>
          <td>

          <form method="post" action="{{url('/Clientes/'.$clientes->id) }}">
               <a href="{{ url('/Clientes/'. $clientes->id)}}"> <img src="{{asset('/images/ver.png')}}"> </a>
               <a href="{{ url('/Clientes/'.$clientes->id.'/edit') }}"><img src="{{asset('/images/editar.png')}}"></a>
               {{csrf_field() }}
               {{method_field('DELETE') }}

               <a type="submit" onclick="return confirm('¿Esta seguro?');"><img src="{{asset('/images/borrar.png')}}"> </a>
               </form>
     </td>
     </tr>
     @endforeach
