...
<br><br><br><br><br>
          ...
          <div class="row">
               ...
               <div class="pull-right">
                    ...
                    <a class="btn btn-success" href="{{ route('descargarPDFproductos') }}"> GENERAR PDF </a>
                    <a class="btn btn-success" href="{{ route('RegistrarCon.create') }}"> REGRESAR </a>
               </div>
          </div>
     </div>
@if ($message = Session::get('Aceptado'))
     ...
     <thead class="thead-light">
          ...
          <tbody>
          @foreach ($datosesiones as $sesiones)
          <tr>

               <td>{{ $sesiones->id}} </td>
               <td>{{ $sesiones->nombre2}} </td>
               <td>{{ $sesiones->paterno }} </td>
               <td>{{ $sesiones->materno }} </td>
               <td>{{ $sesiones->email }} </td>
               <td>{{ $sesiones->edad }} </td>
               <td>{{ $sesiones->fecha }} </td>
               <td>{{ $sesiones->albunes }} </td>
          <td>
               
               <form method="post" action="{{url('/Sesiones/'.$sesiones->id) }}">
               <a href="{{ url('/Sesiones/'. $sesiones->id)}}"> <img src="{{asset('/images/ver.png')}}"> </a>
               <a href="{{ url('/Sesiones/'.$sesiones->id.'/edit') }}"><img src="{{asset('/images/editar.png')}}"></a>
               {{csrf_field() }}
