...
@if ($message = Session::get('Aceptado'))
     ...
     <thead class="thead-light">
          ...
          <tr>
               <td>{{ $producto->id }} </td>
               <td>{{ $producto->nombre3 }} </td>
               <td>{{ $producto->apellidos }} </td>
               <td>{{ $producto->telefono }} </td>
               <td>{{ $producto->dirección }} </td>
          ...
          <form method="post" action="{{url('/Productos/'.$producto->id) }}">
               <a href="{{ url('/Productos/'. $producto->id)}}"> <img src="{{asset('/images/ver.png')}}"> </a>
               <a href="{{ url('/Productos/'.$producto->id.'/edit') }}"><img src="{{asset('/images/editar.png')}}"></a>
               {{csrf_field() }}
               {{method_field('DELETE') }}
